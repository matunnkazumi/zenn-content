...
このshimはツールをインストールしたタイミングで更新しているようです。上記の手順でいうと```asdf install nodejs 16.10.0```を実行した時ですね。この時にダウンロードされたファイルをもとに自動的に作成・更新していると思われます。
...
もし将来的にCorepackの実験的扱いが無くなった時に、このreshimの手順は不要になるんじゃないかと期待していますが、当面は様子見ですねー。
...
```
asdf的には1.22.11をインストールした状態にして、そのyarnのコマンド側の設定でv2以降を使うようにします。
